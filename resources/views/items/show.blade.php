                            ...
                            <div class="lg:w-1/2 md:w-2/3 mx-auto">
                                <div class="flex flex-wrap -m-2">
                                    ...
                                    <form id="delete_{{ $item->id }}"
                                        action="{{ route('items.destroy', ['item' => $item->id]) }}" method="post">
                                        @csrf
                                        @method('delete')
                                        <div class="p-2 w-full">
    ...
    <script>
        function deletePost(e) {
            ...
            if (confirm('本当に削除していいですか？')) {
                document.getElementById('delete_' + e.dataset.id)
                .submit() //id = 「'delete_' + e.dataset.id」のフォームを取得して、submit()
            }
        }
    </script>
</x-app-layout>
